...
namespace TiendaServicios.Api.CarritoCompra.RemoteService
{
    public class LibrosService : ILibrosService
    {
        ...
        public async Task<(bool resultado, LibroRemote Libro, string ErrorMessage)> GetLibro(string IdUnico)
        {
            try
            {
                var cliente = httpClient.CreateClient("Libros");
                var response = await cliente.GetAsync($"api/LibroMaterial/{IdUnico}");
